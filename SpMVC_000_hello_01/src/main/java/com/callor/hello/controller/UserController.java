...
public class UserController {
	...
	public String join() {
		...
		/*
		 * method 에서 String 값을 return 하면 views 폴더에서 해당 String 의 jsp 파일을 찾아 rendering 을
		 * 실행한다
		 * 
		 * method 에서 null 값을 return 하면 views 폴더에서 RequestMapping 과 같은 path 위치에서 jsp 파일을
		 * 찾는다 /views/user/join.jsp 를 찾는다
		 */
		return null;
	}

	/*
	 * Spring 에서 web 에서 여러가지 데이터를 받을 경우 모든 변수를 매개변수로 선언하지 않고 Dto 클래스를 선언하고 Dto 클래스,
	 * 객체를 매개변수로 선언한다
	 * 
	 * @ModelAttribute 속성을 지정한다
	 * 
	 * Spring framework 의 도구들이 form 에서 전달된 데이터를 Dto 객체에 자동으로 담아준다
	 * 
	 * " Java 코딩에서 method 에 전달할 데이터(변수)가 2개를 초과하면 무조건 Dto 를 만들자 "
	 * 
	 * @ModelAttribute 속성은 현재 Spring Framework 에서는 선택사항이지만 오래된 Spring Framework 에서는
	 * 필수 항목이다 그래서 선택사항이지만 부착하는 습관을 들이자
	 * 
	 */
