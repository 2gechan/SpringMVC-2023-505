 ...
 * 이슈들이 많이 발견되었다.
 * 또한 java가 업그레이드 되는 동안에 날짜와 관련된 여러가지
 * 문제를 해결하지 못하는 단점이 있었다.
 * Java 1.8 이후에 LocalDate, LocalTime, LocalDateTime 이라는
 * 새로운 날짜 시간관련 클래스가 새롭게 만들어졌다.
 * 현재 최신의 Java에서는 가급적 Local** 클래스를 사용하여
 * 날짜 관련 문제를 해결하도록 권장하고 잇다.
 * 
 * DB와 연동되는 프로젝트에서 날짜 시간 관련된 데이터를 취급하는데 있어
 * java와 DB 서버간에 데이터 교환에 문제를 일으키기도 한다.
 * 보통 DB와 연동을 할 때 날짜 형식의 데이터를 사용하지 않고
 * 문자열로 변환하여 사용한다.
 */
