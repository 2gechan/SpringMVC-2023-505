...
public class HomeController {
	...
	/*
	 * @Qualifier(QualifierConfig.SERIVCE.FILE_V2) 2개 이상의 Component 들이 같은 Service
	 * interface를 상속받아 생성된 경우 이 Component를 주입받아 사용하는 곳에서는 어떤 Component를 주입받아 사용해야
	 * 하는지 알수가 없다.
	 * 
	 * 이 때는 @Component("이름")에 이름을 부여하고 사용할 곳에서 주입받을 때 @Qualifier()를 통해 명시적으로 어떤
	 * Component를 주입받을지 지정해 주어야 한다.
	 */
	private final FileService fileService;
	private final BBsDao bbsDao;
	private final FileDao fileDao;

	public HomeController(@Qualifier(QualifierConfig.SERIVCE.FILE_V2) FileService fileService, BBsDao bbsDao,
			FileDao fileDao) {
		this.bbsDao = bbsDao;
...
//		List<BBsDto> bbsList = bbsDao.selectAll();
	...
	/*
	 * @ModelAttribute("BBS") GET /insert가 호출될 때 아직 bbsDto 객체는 null인 상태이다 bbsDto가
	 * null인 경우 어딘가에 @ModelAttribute("BBS") 라는 이름이 부착된 method를 찾는다. 만약 method가 발견되면
	 * 자동으로 해당 method를 호출하여 데이터를 준비해 준다.
	 */
...
//		model.addAttribute("BBS", bbsDto);
	...
	/*
	 * @RequestParam(value="b_usrname") String username Client(Browser)에서 b_username
	 * 이라는 변수에 값이 담겨서 전달되어 오면 username 이라는 변수에 값을 저장하여 method 내부로 전달하라
	 * 
	 * Client와 Server 간에 변수 이름이 다를때 사용하는 선택사항
	 */
	@RequestMapping(value = "/insert", method = RequestMethod.POST)
	/*
	 * 매개변수 설명 BBDto bbsDto : input tag에 입력한 게시판 관련 text 를 받는 곳 MultipartFile b_file
...
//		return "input";
	...
	public String detail(String seq, BBsDto bbsDto, Model model) {
