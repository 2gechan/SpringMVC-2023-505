...
public class AddrServiceImplV1 implements AddrService {
	 ...
	 * java의 전통적인 객체 생성 코드와 대비되는
	 * Spring Framework Project의 독특한 기법
	 * 
	 * 객체를 선언만 하고, 의존성 주입 절차를 수행하면
	 * Spring Container에 의해 이미 생성되어
	 * 준비된 객체를 주입해주는 기능
	 * 
	 * 의존성 주입하는 방법은 필드 주입, 생성자 주입, Setter 주입이 있다.
	 */
	 ...
	 * private으로 선언할 것을 권장하는 방법이다.
	 * 객체를 private으로 선언하면 상속(extends)을 했을 때
	...
	/*
	 * 생성자 의존성 주입(Constructor Dependency Inject, DI)
	 * 
	 * 생성자를 통해서 주입될 객체는 final로 선언을 한다.
	 * 이 방식의 특징은 한번 주입이 되면, 코드가 작동되는 중에
	 * 또다시 주입을 할 수 없다. 즉, 객체가 안전하다.
	 * 객체의 순환참조가 없다.
	 * 이 방식은 메모리 누수가 없고, 객체가 보호되며, 순환 참조에 의한
	 * 코드의 Exception을 막는다.
	 * @Autowired를 붙이지 않아도 된다.
	 * 
	 * 이 방식을 사용하려면 반드시 생성자 메서드가 있어야 한다.
	 * 코드가 다소 번잡해 진다
	 * 실수로 생성자 메서드를 만들지 않았을 경우
